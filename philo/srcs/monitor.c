...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/20 04:25:42 by kyuki             #+#    #+#             */
/*   Updated: 2022/08/20 04:28:12 by kyuki            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int8_t	is_expired_time(t_philo *philo)
{
	...
	return (philo->time_last_eat != UNSET && elapsed_time \
		>= philo->data->action_time.time_die);
}
...
{
	if (is_expired_time(philo) || philo->data->died == 1 \
		|| is_expired_eat_times(philo) || philo->data->num_philo == 1)
	{
		if (philo->data->at_least_eat_num_philo != 0)
			do_mtx(&(t_print){philo, DIED}, &philo->data->mtx_print_status, \
				&ft_print_status);
		philo->data->died = 1;
		return (1);
	}
